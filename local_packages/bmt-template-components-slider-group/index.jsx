...
const SliderGroup = (props) => {
    ...
    const keyNumberOfActivatedErrors = `${key}_number-of-activated-errors`;
    const keyActivatedError = `${key}_show-activated-error`;
    const keyValue = `${key}_value`;

    useEffect(() => {
        setState(function(prev){
            if (!prev[keyInitialized]) {
                return {
                    ...prev,
                    [keyInitialized]: true,
                    [keyActivated]: props.defaultValue !== undefined,
                    [keyValue]: props.defaultValue !== undefined ? props.defaultValue  : "",
                    [keyNumberOfActivatedErrors]: 0
                }
            } else {
                return prev
            }
    ...
    return <div className="shadow-md p-5 rounded-md border border-slate-200 my-4">
        ...
        >
            ...
            <div className={"relative"}>
                ...
                {!state[keyActivated] &&
                    <div
                        className={`absolute pointer-events-none z-10 top-0 ${style.activateOverlay} px-4 left-1/2 -translate-x-1/2 font-medium ${state[keyActivatedError] ? "text-red-500" : ""}`}>
                        {t("please-activate")}
                    </div>
                }
            </div>
                ...
                <div className="w-full flex justify-between text-xs">
                    {
                        [...Array((props.max - props.min) / props.step + 1)].map((_, index) => {
                            return (
                                <div className="flex flex-col justify-center w-0 items-center  px-2 text-md"
                                     key={index}>
                ...
                <div className="border rounded p-2 mt-2 font-medium relative">

                    {
                        ...
                        <div className={"flex justify-between"}>
                            <output>
                                <div
                                    className={`transition-opacity duration-700 ${state[keyActivated] ? "opacity-100" : "invisible opacity-0"}`}>
...
// Validation type.
SliderGroup.validationType = "SliderGroup";
